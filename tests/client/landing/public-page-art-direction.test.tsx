...
import { derivePublicLadder, ladderBaseFee, pilotBaseFee } from "@/lib/pricing/team-ladder";
...
describe("the separate contact and pricing art directions", () => {
  ...
  it("the pricing page opens with the two levels, carries the calculator and routes the closing decision to contact", () => {
    ...
    expect(within(tiles).getByText(plain(hufAmount(ladderBaseFee(ladder, "kep"))))).toBeInTheDocument();
    expect(within(tiles).getByText(plain(hufAmount(ladderBaseFee(ladder, "prog"))))).toBeInTheDocument();
    ...
    expect(within(pilot).getByText(plain(hufAmount(pilotBaseFee(ladder, "prog"))))).toBeInTheDocument();
