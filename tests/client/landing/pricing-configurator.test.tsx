...
describe("TeamPricingConfigurator", () => {
  it("a Csapatkép szinttel és 10 fővel nyit, a hirdetett fejenkénti árral", () => {
    ...
    expect(
      screen.getByText(plain(`Összesen ${formatHuf(ladderPrice(ladder, "kep", 10).total)} Ft a csapatra`)),
    ).toBeInTheDocument();
  });
  ...
  it("a Csapatprogramra váltva az ár és a tartalom-lista frissül", () => {
    ...
    expect(
      screen.getByText(plain(`Összesen ${formatHuf(ladderPrice(ladder, "prog", 10).total)} Ft a csapatra`)),
    ).toBeInTheDocument();
  });
