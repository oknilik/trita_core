...
for (const includeTrustNetwork of [false, true]) test(`Team Scan → parallel submissions → review/publish → pinned Follow-up; trust=${includeTrustNetwork}`, async () => {
  ...
  try {
    ...
    const participant = await prisma.campaignParticipant.findUniqueOrThrow({ where: { campaignId_userId: { campaignId: scanId, userId: ids[0] } } });
    const missingObserver = { ...completionMap(participant.stepCompletions) };
    delete missingObserver.OBSERVER_360;
    await prisma.campaignParticipant.update({ where: { id: participant.id }, data: { stepCompletions: { v: 1, activities: missingObserver } as Prisma.InputJsonValue } });
    await assert.rejects(mutateProgramReport({ ...input, action: "review", expectedRevision: 1 }), /REPORT_OBSERVER_DATA_INSUFFICIENT/);
    await assert.rejects(mutateProgramReport({ ...input, action: "review", expectedRevision: 1, fields: { observerOverrideReason: "short" } }), /INVALID_INPUT/);
    const reason = "Discuss incomplete observer coverage explicitly with the team.";
    const override = await mutateProgramReport({ ...input, action: "review", expectedRevision: 1, fields: { observerOverrideReason: reason } });
    assert.equal(override.aggregates?.program?.observerOverride?.reason, reason);
    assert.equal(override.aggregates?.program?.observerOverride?.actorId, ids[0]);
    assert.equal(override.aggregates?.program?.observerCompletedParticipants, 2);
    const audit = await prisma.teamActionEvent.findFirstOrThrow({ where: { reportId: report.id, eventType: "REVIEWED" } });
    assert.equal((audit.payload as { observerOverride: { reason: string } }).observerOverride.reason, reason);
    const insufficient = structuredClone(override.aggregates!);
    insufficient.psychSafety = null;
    assert.equal(programDataError(insufficient), "REPORT_PULSE_DATA_INSUFFICIENT");
    const overridePreview = await mutateProgramReport({ ...input, action: "preview", expectedRevision: override.revision, fields: { observerOverrideReason: "An unsaved alternative rationale for preview only." } });
    assert.equal(overridePreview.aggregates?.program?.observerOverride?.reason, "An unsaved alternative rationale for preview only.");
    assert.equal(overridePreview.reviewedRevision, null);
    assert.equal((await prisma.teamReport.findUniqueOrThrow({ where: { id: report.id } })).reviewedRevision, override.revision);
    const cleared = await mutateProgramReport({ ...input, action: "save", expectedRevision: override.revision, fields: { observerOverrideReason: null } });
    assert.equal(cleared.aggregates?.program?.observerOverride, undefined);
    await assert.rejects(mutateProgramReport({ ...input, action: "publish", expectedRevision: cleared.revision }), /REVIEW_REQUIRED/);
    await assert.rejects(mutateProgramReport({ ...input, action: "review", expectedRevision: cleared.revision }), /REPORT_OBSERVER_DATA_INSUFFICIENT/);
    await prisma.campaignParticipant.update({ where: { id: participant.id }, data: { stepCompletions: participant.stepCompletions as Prisma.InputJsonValue } });
    let reviewed = await mutateProgramReport({ ...input, action: "review", expectedRevision: cleared.revision });
    const frozenBeforePreview = await prisma.teamReport.findUniqueOrThrow({ where: { id: report.id } });
    const preview = await mutateProgramReport({ ...input, action: "preview", expectedRevision: reviewed.revision, fields: { summary: "Unsaved preview" } });
    assert.equal(preview.summary, "Unsaved preview");
    assert.equal(preview.revision, reviewed.revision);
    assert.deepEqual(await prisma.teamReport.findUniqueOrThrow({ where: { id: report.id } }), frozenBeforePreview);
    ...
    await prisma.campaign.update({ where: { id: followId }, data: { programSnapshot: { version: 999 } } });
    const unsupported = await buildTeamReportAggregates(teamId, { assessmentCampaignId: followId });
    assert.ok(unsupported);
    assert.equal(unsupported.program?.unsupported, true);
    assert.equal(programDataError(unsupported), "PROGRAM_SNAPSHOT_REQUIRED");
  } finally {
