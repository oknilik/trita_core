  ...
  return (
    <section className="rounded-2xl border border-sand bg-surface-card p-5 sm:p-6">
      ...
      <div className="mt-6 space-y-5">
        {HEXACO_ORDER.filter((dim) => typeof dimensions[dim] === "number").map(
          (dim) => (
            <div
              key={dim}
            ...
            >
              <span className="col-span-2 text-caption text-ink sm:col-span-1">
                {HEXACO_DIMENSIONS[dim][locale]}
              </span>
              <div
                className="relative mx-2 h-1 rounded-full bg-sand"
                role="img"
                aria-label={`${HEXACO_DIMENSIONS[dim][locale]}: ${dimensions[dim]} / 100${baseline ? `; ${t("candidateProgram.team", locale)}: ${baseline[dim] ?? "–"}` : ""}`}
              >
                <span
