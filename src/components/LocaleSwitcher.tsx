...
export function LocaleSwitcher({ className }: LocaleSwitcherProps) {
  ...
  return (
    <div
      className={`relative flex min-h-[44px] items-center rounded-lg border border-gray-100 bg-white pl-9 pr-7 text-xs font-semibold text-gray-700 ${className ?? ""}`}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 20 20"
        fill="currentColor"
        className="pointer-events-none absolute left-3 h-4 w-4 text-indigo-500"
      >
        <path
          fillRule="evenodd"
      ...
      <span className="pointer-events-none text-xs font-semibold text-gray-700">
        {locale.toUpperCase()}
      </span>
      <svg
        xmlns="http://www.w3.org/2000/svg"
