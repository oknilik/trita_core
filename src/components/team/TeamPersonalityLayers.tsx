  ...
  return <>
    <section className="py-7">
      ...
      <div className="grid gap-7 md:grid-cols-[1fr_1.4fr]">
        <div><p className="text-xs font-semibold uppercase tracking-widest text-sage">{hu ? "02 / Összesített személyiségprofil" : "02 / Aggregated personality"}</p><h2 className="mt-3 font-fraunces text-2xl text-ink sm:text-3xl">{hu ? "Miből épül fel a csapat?" : "What is the team made of?"}</h2><p className="mt-4 text-sm leading-relaxed text-ink-body">{hu ? "Az egyéni személyiségprofilok összesítése. A közös átlag nem jelenti azt, hogy mindenki egyforma." : "Aggregated individual HEXACO profiles. A shared average does not mean everyone is alike."}</p>{typeof count === "number" && dims.length > 0 && <p className="mt-3 text-xs text-muted">{hu ? `${count} egyéni profil` : `${count} individual profiles`}</p>}</div>
        ...
        })}</div><p className="mt-3 text-xs leading-relaxed text-muted">{hu ? "0–100-as skála · A pont az átlagot jelöli. Ahol rendelkezésre áll, a halvány sáv az átlagtól mindkét irányban egy mintaszórásnyi tartományt mutat, a skála határai közé szorítva. Ez nem konfidenciaintervallum." : "0–100 scale · Dot: mean. Where available, the faint band shows one sample SD around the mean, clipped to the scale; it is not a confidence interval."}</p></div> : <p className="text-sm text-muted">{hu ? "Ehhez a riporthoz nem rögzítettünk összesített személyiségprofilt. A hiányzó értékeket nem becsüljük meg a négy tengelyből." : "This report snapshot contains no aggregated HEXACO profile. Missing values are not reconstructed from the four axes."}</p>}
      </div>
    </section>
    <section className="py-7">
      {sourceLabel && <p className="mb-4 rounded-lg border border-sand p-3 text-xs text-muted">{sourceLabel}</p>}
      ...
      {composition ? <><p className="mt-3 text-xs text-muted">{hu ? `${composition.memberCount} profil · Válassz egy tengelyt a forrása megismeréséhez.` : `${composition.memberCount} profiles · Select an axis to see its source.`}</p>
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted">{tr("cohesionCaveat")}</p><p className="mt-2 text-xs leading-relaxed text-muted">{hu ? "A Hajtóerő a társas aktivitás, a Fegyelem pedig a rendszerezettség iránti igény közelítő mutatója. A személyiségbeli Nyitottság nem azonos a megvalósítás rugalmasságával. Az Emocionalitás értékét nem használjuk a négy tengely számításában." : "Drive approximates social activity; Discipline reflects a tendency toward organization. Personality Openness is not the same as adaptive implementation. Emotionality does not enter this four-axis derivation."}</p>
