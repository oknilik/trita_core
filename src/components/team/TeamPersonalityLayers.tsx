...
export function TeamPersonalityLayers({ composition, averages, spread, count, locale, legacyPattern }: {
  ...
  return <>
    ...
    <section className="py-7">
      ...
      {composition ? <><p className="mt-3 text-xs text-muted">{hu ? `${composition.memberCount} profil · Válassz egy tengelyt a forrása megismeréséhez.` : `${composition.memberCount} profiles · Select an axis to see its source.`}</p>
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted">{tr("cohesionCaveat")}</p><p className="mt-2 text-xs leading-relaxed text-muted">{hu ? "A Hajtóerő társas aktivitást közelít; a Fegyelem rendszerezettségi hajlamot. A személyiségbeli Nyitottság nem azonos az adaptív végrehajtással. Az Emocionalitás nem vesz részt ebben a négytengelyes képzésben." : "Drive approximates social activity; Discipline reflects a tendency toward organization. Personality Openness is not the same as adaptive execution. Emotionality does not enter this four-axis derivation."}</p>
        <p className="mt-5 border-t border-sand pt-4 text-sm text-ink-body">{tr("stable")}: <strong className="font-medium text-ink">{composition.name}</strong>{composition.stability !== "stabil" && <span className="mt-2 block text-xs text-muted">{tr("compositionTentative")}</span>}</p>
      </> : <p className="mt-4 text-sm text-muted">{legacyPattern ? `${tr("stable")}: ${legacyPattern}. ` : ""}{tr("noComposition")}</p>}
    </section>
  </>;
}
