...
export function NotificationBell({ isOpen, onToggle }: NotificationBellProps) {
  ...
  return (
    ...
    >
      ...
      {count > 0 && (
        <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center lg:-right-0.5 lg:-top-0.5 rounded-full bg-[var(--color-bronze-dark)] px-1 text-micro font-bold leading-none text-white">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </button>
