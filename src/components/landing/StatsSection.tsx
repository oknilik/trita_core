...
export function StatsSection({ locale }: { locale: Locale }) {
  ...
  return (
    <section className="border-b border-sand bg-ink px-6 lg:px-16">
      <div className="grid grid-cols-2 md:grid-cols-4">
        {stats.map((item, index) => (
          ...
          >
            <div className="font-fraunces mb-1 text-[28px] leading-none font-black tracking-[-1px] text-white lg:text-4xl">
              {item.value}
            </div>
            <div className="font-dm-sans px-2 text-[11px] tracking-[0.5px] text-white/40">
              {item.label}
            </div>
          </div>
        ))}
