...
// A landing két riport-előnézete: a valódi eredménynézet (SelfPanel) és a
// publikált csapatkép (TeamPanel) kicsinyített mása. Egy helyen élnek, mert
// a főoldal hero-ja és csapat-átvezetője, valamint a /team-dynamics hero-ja
// ugyanazt a két kártyát mutatja — másolat helyett közös forrásból.
...
export function SelfPanel() {
  ...
  return (
    <div className="overflow-hidden rounded-[28px] bg-surface-card shadow-[0_24px_64px_rgba(26,26,46,0.10)]">
      {/* ═══ SÖTÉT HERO FEJLÉC ═══ */}
          ...
          <div className="overflow-hidden rounded-xl border border-[var(--color-border-soft)] bg-surface-card">
            {likelyRoles.map((role, index) => {
              ...
              return (
                <div
                  key={role}
                  data-role-rank={rankVisual.rank}
...
export function TeamPanel() {
  ...
  const dims = [
    ...
    { name: t("landing.teamAxisOpenness", locale), mean: 56, color: "var(--color-dim-h-base)" },
  ];
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 gap-1.5">
            <span className="rounded-full bg-[var(--color-surface-subtle)] px-2 py-1 text-micro text-muted">
              {t("landing.teamPanelMembers", locale)}
            </span>
            <span className="rounded-full bg-[var(--color-surface-subtle)] px-2 py-1 text-micro text-muted">
              100%
            </span>
