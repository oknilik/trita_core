...
/**
 * GET /api/team-roles/peers?campaignId=…
 *
 * A rater számára: a kampány cél-csapatának értékelendő tagjai + melyikről
 * adott már be visszajelzést EBBEN a körben. Guard: aktív kampány,
 * résztvevő, és a TEAM_ROLE_360 az aktuálisan nyitott lépése.
 */
export async function GET(req: NextRequest) {
  ...
  const participant = await prisma.campaignParticipant.findUnique({
    ...
    select: {
      ...
      campaign: {
        select: { id: true, status: true, type: true, steps: true, programSnapshot: true, teamId: true, teamIds: true },
      },
    },
  });
