...
/**
 * POST /api/team-roles/peers/submit — csapattársi szerep-visszajelzés(ek)
 * beküldése (batch: a kliens akár személyenként, akár egyben küldhet).
 *
 * Guard-ok: aktív kampány + résztvevő + nyitott TEAM_ROLE_360 lépés
 * (STEP_LOCKED, 409) + minden értékelt a cél-csapat tagja. Upsert:
 * ismételt beküldés ugyanarról a személyről felülírja a korábbit
 * (kör-önmagában — a unique kulcs kampány-szintű).
 *
 * Ha a rater ezzel lefedte a csapat minden tagját, a TEAM_ROLE_360 lépés
 * teljesül (idempotens léptető + következő lépés értesítése).
 */
export async function POST(req: NextRequest) {
  ...
  for (const obs of observations) {
    ...
    if (!isValidTeamRoleSelectionSet(obs.selections)) {
  ...
  if (!isStepOpenFor(participant.campaign, participant, "TEAM_ROLE_360")) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    for (const obs of observations) {
      await tx.teamRoleObservation.upsert({
        ...
        update: { teamId: guard.teamId, selections: obs.selections },
      });
    }
    const rated = await tx.teamRoleObservation.findMany({
