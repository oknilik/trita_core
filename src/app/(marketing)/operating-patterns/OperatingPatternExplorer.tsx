  ...
  return <main className="bg-cream px-4 pb-16 pt-10 text-ink sm:px-6 sm:pt-16">
    <div className="mx-auto max-w-5xl">
      <div className="mb-10 grid items-center gap-8 md:grid-cols-[1fr_160px]">
        <div><p className="text-xs font-semibold uppercase tracking-widest text-sage">{hu ? "Csapatkép · Team patterns" : "Team picture · Team patterns"}</p>
          <h1 className="mt-4 font-fraunces text-4xl leading-tight tracking-tight sm:text-5xl">{hu ? "Sokféleképpen lehet együtt működni." : "There are many ways to work together."}</h1>
          <p className="mt-5 max-w-2xl text-base leading-relaxed text-ink-body">{hu ? "Négy tengely. Tizenhat működési minta. Fedezd fel, hogyan áramlik az információ, hogyan hangolódik össze a munka, hol születnek a döntések és hogyan halad a megvalósítás." : "Four axes. Sixteen operating patterns. Explore how information flows, work is coordinated, decisions are made and tasks are carried out."}</p>
        </div><OperatingPatternMark code="SODA" className="hidden w-36 text-ink md:grid" />
      </div>
      <div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-fraunces text-2xl">{hu ? "A működési minták térképe" : "The operating pattern map"}</h2><p className="mt-2 text-sm text-muted">{hu ? "Válassz egy mintát, és ismerd meg közelebbről." : "Choose a pattern to explore it."}</p></div><span className="rounded-full border border-sand px-3 py-1 text-xs text-muted">{hu ? "Kísérleti tipológia" : "Experimental typology"}</span></div>
