  ...
  return <main className="bg-cream px-4 pb-16 pt-10 text-ink sm:px-6 sm:pt-16">
    <div className="mx-auto max-w-5xl">
      ...
      <section ref={mapRef} id="operating-map" aria-labelledby="operating-map-title" className="scroll-mt-28">
        <p className="text-xs font-semibold uppercase tracking-widest text-sage">{hu ? "Közös kiindulópont a beszélgetéshez" : "A shared map for the conversation"}</p>
        ...
        <div className="mb-4 mt-6 grid grid-cols-2 gap-3 lg:grid-cols-4">
          {OPERATING_FAMILIES.map((f) => <div key={f.code} className={`min-w-0 rounded-2xl p-4 ${f.tone}`}>
            ...
            <p className="mt-2 text-xs leading-relaxed text-ink-body">{hu
              ? `${f.code[0] === "S" ? "Rendezett tudás" : "Közvetlen információ"}, ${f.code[1] === "E" ? "tisztázott szerepek" : "kialakult összhang"}.`
      ...
      <details className="mt-7 border-t border-sand pt-3"><summary className="min-h-11 cursor-pointer py-3 text-sm font-semibold">{hu ? "Hogyan olvasd ezt a térképet?" : "How to read this map"}</summary><div className="max-w-3xl space-y-3 text-sm leading-relaxed text-ink-body"><p>{hu ? "A kód sorrendje: információáramlás → koordináció → döntéshozatal → megvalósítás. Az első két betű adja a családot. A családon belül a döntési jogkör és a megvalósítás módja különbözteti meg a négy mintát." : "The code order is information flow → coordination → decision-making → implementation. The first two letters define the family; decision authority and implementation distinguish its four patterns."}</p><p>{hu ? "A csapatkép az adott időszak működését írja le. Nem személyiségtípus vagy rangsor. A csapat személyiség-összetételét külön vizsgáljuk. Attól, hogy a munkátok összehangolása főként a kialakult szokásokra épül, még nem biztos, hogy a döntési jogkörök is megoszlanak." : "This describes how a team operates during a particular period, not a personality type or ranking. Personality composition is a separate layer. Organic coordination alone does not imply distributed decisions."}</p><p>{hu ? "A négy területet folytonos skálákon mérjük. A riport külön jelzi, ha az eredmény a skála közepéhez közeli, a válaszok jelentősen eltérnek, vagy kevés az adat. A modell kísérleti, a mintázatok érvényességét még nem igazoltuk kutatással." : "Measured axes are continuous. Near-midpoint, divergent or incomplete results are flagged in the report. This is an experimental, unvalidated typology."}</p></div></details>
        </details>
      </section>
    </div>
  </main>;
