...
export async function mutateProgramReport(input: {
  ...
  const source = await prisma.teamReport.findUnique({ where: { id: input.reportId }, include: { campaign: true } });
  if (!source || source.teamId !== input.teamId || !source.campaign?.programSnapshot) throw new ProgramReportError("NOT_FOUND");
  if (source.revision !== input.expectedRevision) throw new ProgramReportError("REPORT_REVISION_CONFLICT");
  const rebuild = !["publish", "unpublish"].includes(input.action);
  if (rebuild && source.campaign.status !== "CLOSED") throw new ProgramReportError("REPORT_CAMPAIGN_NOT_CLOSED");
  // Aggregation may acquire several connections. Never hold a transaction/row lock while doing it.
  const aggregates = rebuild ? await buildTeamReportAggregates(source.teamId, { assessmentCampaignId: source.campaignId! }) : null;
  return prisma.$transaction(async tx => {
    ...
    if (r.campaignId !== source.campaignId || input.expectedRevision !== r.revision) throw new ProgramReportError("REPORT_REVISION_CONFLICT");
