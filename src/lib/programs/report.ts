...
export function programDataError(agg: TeamReportAggregates): string | null {
  ...
  if (!op || AXES.some(a => op.axes[a].status !== "available" || op.axes[a].coverage < 0.6)) return "REPORT_OPERATING_DATA_INSUFFICIENT";
  if (!agg.psychSafety) return "REPORT_PULSE_DATA_INSUFFICIENT";
  if (!agg.dimensionAverages || agg.completedCount < 3) return "REPORT_SELF_DATA_INSUFFICIENT";
...
export function compareOperating(current: TeamReportAggregates, baseline: TeamReportAggregates) {
  ...
  return AXES.flatMap(axis => {
    ...
    return x.status === "available" && y.status === "available" && x.coverage >= 0.6 && y.coverage >= 0.6 && x.mean !== null && y.mean !== null
