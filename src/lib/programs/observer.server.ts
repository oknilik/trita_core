...
export async function reconcileProgramObserver(tx: Prisma.TransactionClient, campaignId: string, userId: string, snapshot: unknown) {
  ...
  await tx.campaignParticipant.update({ where: { id: p.id }, data: { stepCompletions: programCompletions(p.stepCompletions, "OBSERVER_360") as unknown as Prisma.InputJsonValue } });
}
