...
export async function attachProgramEvidence(teamId: string, campaignId: string | undefined, current: TeamReportAggregates): Promise<TeamReportAggregates> {
  ...
  current.program = { key: p.key, participantCount: campaign.participants.length, observerReady: campaign.participants.length >= p.policy.minRespondents && campaign.participants.every(x => Boolean(completionMap(x.stepCompletions).OBSERVER_360)) };
  if (p.key === "FOLLOW_UP") {
