import { POLICY } from "@/lib/team-operating-style/policy";
import { z } from "zod";
...
const activitySchema = z
  .object({
    key: z.enum(ACTIVITY_KEYS),
    scope: z.enum(["participant", "team"]),
    required: z.boolean(),
    dependencies: z.array(z.enum(ACTIVITY_KEYS)),
  })
  .strict();
export const programSchema = z
  .object({
    key: z.enum(PROGRAM_KEYS),
    version: z.literal(1),
    activities: z.array(activitySchema).min(1),
    policy: z
      .object({
        observerResponsesPerParticipant: z.number().int().min(1).max(100),
        minRespondents: z.number().int().min(POLICY.minRespondents).max(10000),
        minOperatingCoverage: z.number().min(POLICY.minCoverage).max(1),
      })
      .strict(),
  })
  .strict();
export const DEFAULT_PROGRAM_POLICY = {
  observerResponsesPerParticipant: 3,
  minRespondents: POLICY.minRespondents,
  minOperatingCoverage: POLICY.minCoverage,
};
...
export function parseProgram(value: unknown): ProgramSnapshot | null {
  ...
  if (keys.size !== p.activities.length)
    throw new Error("PROGRAM_DUPLICATE_ACTIVITY");
  ...
  function visit(key: ActivityKey) {
    ...
    for (const dep of a.dependencies) {
      if (
        a.scope === "participant" &&
        p.activities.find((a) => a.key === dep)?.scope === "team"
      )
        throw new Error("PROGRAM_INVALID_SCOPE");
      visit(dep);
    }
    visiting.delete(key);
    visited.add(key);
...
export function safeParseProgram(value: unknown): ProgramSnapshot | null {
  try {
    return parseProgram(value);
  } catch {
    return null;
  }
}
export function createProgramSnapshot(
  key: ProgramKey,
  options: { includeTrustNetwork?: boolean } = {},
): ProgramSnapshot {
  const measurement: ActivityKey[] =
    key === "TEAM_SCAN"
      ? [
          "SELF_ASSESSMENT",
          "TEAM_OPERATING_STYLE",
  ...
  return parseProgram({
    ...
    activities: [
      ...measurement.map((k) => ({
        ...
        required: true,
        dependencies:
          key === "TEAM_SCAN" && k !== "SELF_ASSESSMENT"
            ? ["SELF_ASSESSMENT"]
            : [],
      })),
      ...(options.includeTrustNetwork
        ? [
            {
              key: "TRUST_360",
              scope: "participant",
              required: false,
              dependencies: key === "TEAM_SCAN" ? ["SELF_ASSESSMENT"] : [],
            },
          ]
        : []),
      {
      ...
      },
      {
        ...
        scope: "team",
        required: true,
        dependencies: ["CONSULTANT_REVIEW"],
      },
    ],
...
export interface ProgramCompletions {
  v: 1;
...
export function completionMap(value: unknown): Record<string, unknown> {
  ...
  return Object.fromEntries(
    Object.entries(raw).filter(([key]) => key !== "__program"),
  );
}
...
export function activityStates(
  ...
  return participantActivities(p).map((a) => {
    if (
      ...
      (a.key === "OBSERVER_360" &&
        (facts.observerResponses ?? 0) >=
          p.policy.observerResponsesPerParticipant)
    )
      return { key: a.key, state: "COMPLETED" };
    if (a.dependencies.some((d) => !done[d]))
      return { key: a.key, state: "LOCKED" };
    if (a.key === "OBSERVER_360" && (facts.observerSent ?? 0) > 0)
      return { key: a.key, state: "WAITING" };
    return {
      key: a.key,
      state: facts.started?.includes(a.key) ? "IN_PROGRESS" : "AVAILABLE",
    };
  });
}
export function programActivityLink(key: string, campaignId: string) {
  const paths: Record<string, string> = {
    ...
    OBSERVER_360: "/tasks/observers",
    TRUST_360: "/assessment/trust",
  };
  return `${paths[key] ?? "/tasks"}?campaignId=${encodeURIComponent(campaignId)}`;
}
export const PROGRAM_LABELS = {
  TEAM_SCAN: { hu: "Team Scan", en: "Team Scan" },
  FOLLOW_UP: { hu: "Follow-up", en: "Follow-up" },
};
