...
> mi valósult meg a `codex/ux-journey-simplification` branchen, és mi került
> indoklással backlogra. A 2026-08-12-i lezáró kör a korábbi backlogot is
> összevetette a tényleges kóddal.
...
| A3 | „azonnal megtekintheted" ígéret után 5 képernyős fal (auth→claim→onboarding 2 lépés→dashboard) | claim-út újrarendezése: gyors aktiválás név + consent mezőkkel, közvetlen eredménycél | ✅ KÉSZ (2026-08-12: minimális aktiválási út) |
...
| A15 | Team-mód: history-szennyező toggle (push), nem perzisztens, 3 különböző team-CTA | router.replace + mode-aware fejléc-CTA | ✅ KÉSZ (közös `useSiteMode`, perzisztens mód és kontextuális fejléc-CTA) |
...
| # | Lelet (evidencia) | Javítás | Státusz |
|---|---|---|---|
| B1 | A teljes riport egyetlen scroll, térkép nélkül (~15 blokk, `ProfileTabs.tsx:1030-1085`) | szekció-ugró sáv + WorkStyle expander | ✅ KÉSZ (fejezettérkép + igény szerint nyitható munkastílus/fejlődés) |
...
| B11 | Archetípus-választó: 30 kombináció egyszerre, előre kitöltött idegen elemzéssel | rétegzett feltárás (második mező választás után) | ✅ KÉSZ (előbb fő működésmód, majd motívum; elemzés csak két explicit döntés után) |
...
| B14 | Invite-form menet közben átcímkézi magát (colleagues fetch után) | stabil cím + skeleton | ✅ KÉSZ (`InvitationsTab`: explicit `colleaguesLoading`, helytartó skeleton) |
| B15 | Comparison üres-állapot CTA teljes router-navigáció ugyanarra az oldalra | scroll-to-anchor | ✅ KÉSZ |
| B16 | Notification-panel: dismiss undo nélkül; üres állapot akció nélkül | üres állapotba next-step sor + késleltetett törlés visszavonással | ✅ KÉSZ (`pendingDismiss`, fókuszkezelt undo-ablak) |
...
| B19 | 2 tabhoz túlméretezett tab-gépezet (scroll-fade, auto-center, router.push ugrás) | egyszerű szegmens-gombok | ✅ KÉSZ (kétoszlopos tab, History API; nincs fade, auto-center vagy lapugrás) |
...
- A korábbi backlog minden tétele elkészült vagy a tényleges kódban már kész
  állapotúnak bizonyult; a táblázat ezt a 2026-08-12-i állapotot rögzíti.
